...
namespace SGA
{
	...
	MetaPortfolioRHEAEvaluator::MetaPortfolioRHEAEvaluator(std::vector<int> popSizeCandidates,
	...
	) : Evaluator("MetaPortfolioRHEA"
	),
		_popSizeCandidates(popSizeCandidates), _horizonCandidates(horizon),
		_mutationRateCandidates(mutationRateCandidates), _tournamentSize(tournamentSize),
		_elitism(elitism), _continueSearch(continueSearch), _nrOfChanges(nrOfChanges),
		...
		config(&config), agents(agentsFromConfig(config))
    {
		...
		searchSpaceDims.emplace_back(horizon.size());
    ...
    std::vector<float> MetaPortfolioRHEAEvaluator::evaluate(std::vector<int> point, int nSamples)
    {
		float value = 0;
	...
	float MetaPortfolioRHEAEvaluator::evaluateGame(std::vector<int> point, int opponentID, bool playFirst)
    {
		...
		params.HORIZON = _horizonCandidates[point[1]];
		...
		params.NR_OF_SCRIPT_CHANGES = _nrOfChanges[point[6]];
		...
		auto agentToEvaluate = std::make_unique<MetaPOAgent>(std::move(params));
	...
	void MetaPortfolioRHEAEvaluator::printPoint(const std::vector<int>& point)
    {
	    std::cout << _popSizeCandidates[point[0]] << ", ";
	    std::cout << _horizonCandidates[point[1]] << ", ";
		...
		std::cout << _nrOfChanges[point[6]];
		std::cout << _p1[point[7]];
		std::cout << _p2[point[8]];
		std::cout << _p3[point[9]];
		std::cout << _p4[point[10]];
		std::cout << _p5[point[11]];
		std::cout << _p6[point[12]];
    }
