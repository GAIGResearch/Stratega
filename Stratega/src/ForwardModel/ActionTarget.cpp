...
namespace SGA
{
	...
	int ActionTarget::getPlayerID() const
	{
		...
		throw std::runtime_error("ERROR: Target type " + std::to_string(int(targetType)) + " not recognised in action target.");
	}
	...
	Player& ActionTarget::getPlayer(GameState& state) const
	{
		if (targetType == Type::PlayerReference)
		{
			...
			if (player == nullptr)
			{
				throw std::runtime_error("ERROR: An action-target contained an not existing player with id " + std::to_string(data.playerID));
			}
			return *player;
		}
		else
