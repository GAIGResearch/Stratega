...
namespace SGA {
   ...
   public:
      ...
      // tree policy phase
      UnitMCTSNode* treePolicy(TBSForwardModel& forwardModel, MCTSParameters& params, std::mt19937& randomGenerator, std::map<int, std::vector<UnitMCTSNode*> >* depthToNodes, std::map<int, std::vector<double> >* absNodeToStatistics);
      UnitMCTSNode* expand(TBSForwardModel& forwardModel, MCTSParameters& params, std::mt19937& randomGenerator, std::map<int, std::vector<UnitMCTSNode*> >* depthToNodes);
      UnitMCTSNode* uct(MCTSParameters& params, std::mt19937& randomGenerator, std::map<int, std::vector<double> >* absNodeToStatistics);
      ...
      int applyActionToGameState(TBSForwardModel& forwardModel, GameState& targetGameState, Action& action, MCTSParameters& params) const;
   ...
   public:
      ...
      UnitMCTSNode(TBSForwardModel& forwardModel, GameState gameState, std::vector<int> unitIndex_, int unitThisStep_, int playerID, int nodeID_);

      //void setRootGameState(shared_ptr<TreeNode> root);
      void searchMCTS(TBSForwardModel& forwardModel, MCTSParameters& params, std::mt19937& randomGenerator, std::map<int, std::vector<UnitMCTSNode*> >* depthToNodes, std::map<int, std::vector<double> >* absNodeToStatistics);
   ...
   private:
      ...
      UnitMCTSNode(TBSForwardModel& forwardModel, GameState gameState, UnitMCTSNode* parent, int childIndex, std::vector<int> unitIndex_, int unitThisStep_, int playerID, int nodeID_);
