...
namespace SGA
{
	...
	std::vector<float> STSAEvaluator::evaluate(std::vector<int> point, int nSamples)
	{
	...
	void STSAEvaluator::printPoint(const std::vector<int>& point)
    {
	...
	std::vector<std::unique_ptr<Agent>> STSAEvaluator::generateAgents() {
        ...
        params.IS_ACTION_INDEPENDENT = true;
        ...
        */
		...
		allAgents[0] = std::make_unique<UnitMCTSAgent>("STSAEvaluator", std::move(params));
		return allAgents;
	}
}
