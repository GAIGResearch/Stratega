...
namespace SGA
{
	...
	RandElasMCTSuEvaluator::RandElasMCTSuEvaluator(
	...
	: Evaluator("RandElasMCTSuEvaluator"),
	...
	std::vector<float> RandElasMCTSuEvaluator::evaluate(std::vector<int> point, int nSamples)
	{
	...
	void RandElasMCTSuEvaluator::printPoint(const std::vector<int>& point)
    {
		std::cout << "K=" << (k_values[point[0]]) << ", ";
		std::cout << "RL=" << (rollout_length[point[1]]) << ", ";
        std::cout << "early_stop=" << (early_stop[point[2]]) << ", ";
		//std::cout << "OS=" << (point[2]) << ";";
    }

	std::vector<std::unique_ptr<Agent>> RandElasMCTSuEvaluator::generateAgents() {
        ...
        params.IS_ACTION_INDEPENDENCE = false;
        params.RANDOM_ABSTRACTION = true;

		params.K = k_values[currentPoint[0]];

		params.ROLLOUT_LENGTH = rollout_length[currentPoint[1]];

        params.absBatch = early_stop[currentPoint[2]];
