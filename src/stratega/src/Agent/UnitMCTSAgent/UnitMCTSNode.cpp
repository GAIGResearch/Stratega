#include <Stratega/Agent/UnitMCTSAgent/MCTSNode.h>
...
MCTSNode::MCTSNode(
   TBSForwardModel& forwardModel, GameState gameState, MCTSNode* parent, const int childIndex)
...
void MCTSNode::searchMCTS(
   TBSForwardModel& forwardModel, MCTSParameters& params, std::mt19937& randomGenerator)
{
   int numIterations = 0;
   bool stop = false;
   ...
   while(! stop) {
      ...
      const double
         delta = deltaAll / Nrollout;  //= selected->rollOut(forwardModel, params, randomGenerator);
      // cout << "delta: " << delta << "\n";
   ...
   TBSForwardModel& forwardModel, MCTSParameters& params, std::mt19937& randomGenerator)
{
   ...
   while(! cur->gameState.isGameOver)  // && cur->nodeDepth < params.ROLLOUT_LENGTH)
   {
      if(! cur->isFullyExpanded()) {
         ...
         double meanQ = 0;  // cur->value/cur->nVisits;

         /* // commented 7/16 to use node.value rather than the mean Q
         for (int i = 0; i < children.size(); i++)
         {
                 meanQ += children[i]->value / children[i]->nVisits;
         }
         if (children.size() > 0)
                 meanQ /= children.size();
         //*/
   ...
   TBSForwardModel& forwardModel, MCTSParameters& params, std::mt19937& /*randomGenerator*/)
{
   ...
   applyActionToGameState(forwardModel, gsCopy, actionSpace.at(new_childIndex), params);

   // generate child node and add it to the tree
   children.push_back(std::unique_ptr< MCTSNode >(
      new MCTSNode(forwardModel, std::move(gsCopy), this, new_childIndex)));
...
MCTSNode* MCTSNode::uct(MCTSParameters& params, std::mt19937& randomGenerator)
{
   ...
   for(size_t i = 0; i < children.size(); ++i) {
      ...
      double uctValue = childValue
                        + params.K
                             * sqrt(log(this->nVisits + 1) / (child->nVisits + params.EPSILON));

      uctValue = noise(
         uctValue,
         params.EPSILON,
         params.doubleDistribution_(randomGenerator));  // break ties randomly
      childValues[i] = uctValue;
   }

   int which = -1;
   double bestValue = iAmMoving ? -std::numeric_limits< double >::max()
   ...
   if(params.ROLLOUTS_ENABLED) {
      ...
      while(! (rolloutFinished(gsCopy, rolloutDepth, params) || gsCopy.isGameOver)) {
         ...
         applyActionToGameState(
            forwardModel, gsCopy, actions.at(randomDistribution(randomGenerator)), params);
         rolloutDepth++;
      }
      return normalize(
         params.STATE_HEURISTIC->evaluateGameState(forwardModel, gsCopy, params.PLAYER_ID), 0, 1);
   }

   return normalize(
      params.STATE_HEURISTIC->evaluateGameState(forwardModel, gameState, params.PLAYER_ID), 0, 1);
}
...
void MCTSNode::applyActionToGameState(
   TBSForwardModel& forwardModel,
   GameState& targetGameState,
   Action& action,
   MCTSParameters& params) const
{
   ...
   while(targetGameState.currentPlayer != params.PLAYER_ID && ! targetGameState.isGameOver) {
      if(params.OPPONENT_MODEL)  // use default opponentModel to choose actions until the turn has
                                 // ended
      {
         params.REMAINING_FM_CALLS--;
      ...
      {
         forwardModel.advanceGameState(
            targetGameState, Action::createEndAction(targetGameState.currentPlayer));
      }
   }
}
...
void MCTSNode::get_branching_number(std::vector< int >* v, int* n)
{
   *n += 1;
   // if (children.size() != 0)
   if(childExpanded != 0) {
